            ...
            for classification, companies in results.items():
                ...
                if companies:
                    ...
                    for company in companies:
                        ...
                        # Add AI scoring columns for No SF Match and Qualified Prospects
                        if classification in ['no_salesforce_match', 'salesforce_qualified']:
                            if 'ai_score' in company:
                                record['AI Score'] = company.get('ai_score', 'N/A')
                            if 'ai_category' in company:
                                record['AI Category'] = company.get('ai_category', 'N/A')
                            if 'ai_use_case' in company:
                                record['AI Use Case'] = company.get('ai_use_case', 'N/A')
                            if 'ai_reasoning' in company:
                                record['AI Reasoning'] = company.get('ai_reasoning', 'N/A')
                    ...
                    elif classification in ['no_salesforce_match', 'salesforce_qualified']:
                        df = pd.DataFrame(columns=['Company', 'Domain', 'AI Score', 'AI Category', 'AI Use Case', 'AI Reasoning'])
                    else:
...
class SalesforceClassifier:
    ...
    def search_by_company(self, company: str) -> Optional[Dict]:
        """
        Search Salesforce for company/account with EXACT matching only
        Only returns a match if we find an exact match
        """
        if not self.sf:
            return None

        try:
            # Use SOQL for exact matching (case-insensitive)
            company_clean = company.strip()

            # Search for exact match
            soql_query = f"SELECT Id, Name, Website, Customer_Designation__c, Owner.Name, LastActivityDate, SystemModstamp FROM Account WHERE Name = '{company_clean}' LIMIT 1"

            print(f"    Salesforce: Searching for exact match '{company_clean}'...")
            result = self.sf.query(soql_query)

            if result.get('records'):
                record = result['records'][0]
                print(f"    Salesforce: Found exact match '{record.get('Name')}'")
                return {
                    'type': 'Account',
                    'id': record['Id'],
                    'name': record.get('Name'),
            ...
            print(f"    Salesforce: No exact match found for '{company}'")
            return None
...
class CompanyProcessor:
    ...
    def process_file(self, csv_file_path: str):
        ...
        for i, company_data in enumerate(companies):
            ...
            # Phase 3: AI Relevancy Scoring (for No SF Match and Qualified Prospects)
            if classification in ['no_salesforce_match', 'salesforce_qualified']:
                progress['phase'] = 'ai_relevancy_scoring'
                self.progress_tracker.save_progress(progress)

                print("  Phase 3: AI Relevancy Scoring...")
                try:
                    from ai_relevancy_scorer import AIRelevancyScorer
                    if not hasattr(self, 'ai_scorer'):
                        self.ai_scorer = AIRelevancyScorer()

                    score, category, use_case, reasoning = self.ai_scorer.score_company(company_name, domain)

                    print(f"    🤖 AI Score: {score}/5")
                    print(f"    📁 Category: {category}")
                    print(f"    💡 Use Case: {use_case}")

                    company_data['ai_score'] = score
                    company_data['ai_category'] = category
                    company_data['ai_use_case'] = use_case
                    company_data['ai_reasoning'] = reasoning

                except Exception as e:
                    print(f"    ⚠️  AI Scoring skipped: {str(e)}")
                    company_data['ai_score'] = 'N/A'
                    company_data['ai_category'] = 'N/A'
                    company_data['ai_use_case'] = 'N/A'
                    company_data['ai_reasoning'] = 'Scoring unavailable'
