...
class SalesforceClassifier:
    ...
    def search_by_company(self, company: str) -> Optional[Dict]:
        """
        Search Salesforce for company/account using fuzzy matching
        Tries multiple variations of the company name
        """
        if not self.sf:
            return None
        ...
        try:
            ...
            for variation in variations:
                # Use LIKE for fuzzy matching - searches for accounts containing this variation
                # Escape single quotes in the variation
                variation_escaped = variation.replace("'", "\\'")
                soql_query = f"SELECT Id, Name, Website, Customer_Designation__c, Owner.Name, LastActivityDate, SystemModstamp FROM Account WHERE Name LIKE '%{variation_escaped}%' LIMIT 5"

                print(f"    Salesforce: Searching for variation '{variation}'...")
                result = self.sf.query(soql_query)

                if result.get('records'):
                    # Return first match
                    record = result['records'][0]
                    print(f"    Salesforce: Found match '{record.get('Name')}' for variation '{variation}'")
                    return {
                        'type': 'Account',
                        'id': record['Id'],
                        'name': record.get('Name'),
