...
# Load environment variables relative to this file so CSVs can be processed
# even when the script is executed from another directory.
from dotenv import load_dotenv

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
ENV_PATH = os.path.join(BASE_DIR, '.env')
if not load_dotenv(dotenv_path=ENV_PATH, override=True):
    print(f"Warning: Could not load environment variables from {ENV_PATH}.")
...
class ProgressTracker:
    ...
    def load_progress(self) -> Dict:
        ...
        return {
            ...
            'processed_count': 0,
            'phase': 'not_started',
            'domain_stats': {'found': 0, 'not_found': 0, 'unverified': 0},
            ...
            for classification, companies in results.items():
                ...
                if companies:
                    ...
                    for company in companies:
                        ...
                        domain_verified = company.get('domain_verified')
                        if domain_verified is not None:
                            record['Domain Verified'] = 'Yes' if domain_verified else 'No'
                        if company.get('domain_guess'):
                            record['Domain Guess'] = company.get('domain_guess', '')
                        if company.get('domain_notes'):
                            record['Domain Notes'] = company.get('domain_notes', '')
                    ...
                    base_columns = ['Company', 'Domain', 'Domain Verified', 'Domain Guess', 'Domain Notes']
                    if classification == 'excluded':
                        df = pd.DataFrame(columns=base_columns + ['Reason'])
                    elif classification == 'current_customers':
                        df = pd.DataFrame(columns=base_columns + ['Relationship Owner', 'Account ID', 'Account URL'])
                    elif classification == 'open_opportunities':
                        df = pd.DataFrame(columns=base_columns + ['Opportunity Owner', 'Opportunity ID', 'Opportunity URL'])
                    else:
...
class DomainDiscovery:
    def __init__(self, log_dir: str):
        ...
        # Cache verification results to avoid repeated network calls
        self._domain_cache: Dict[str, Tuple[bool, Optional[str]]] = {}

    def find_domain(self, company: str) -> Tuple[Optional[str], bool, str]:
        """
        Find company domain/website with improved typo correction and variations.
        Returns: (domain_value, verified_bool, detailed_notes)
        """
        notes: List[str] = []

        normalized_name = company.strip()

        # Step 1: Check if the company name is already a domain (e.g., example.com)
        if '.' in normalized_name and ' ' not in normalized_name:
            potential_domain = normalized_name.lower()
            if potential_domain.startswith('www.'):
                potential_domain = potential_domain[4:]

            verified, cleaned = self._verify_domain(potential_domain)
            if verified and cleaned:
                notes.append(f"Company name treated as domain and verified: {cleaned}")
                return cleaned, True, "; ".join(notes)

            notes.append(f"Company name resembled a domain but verification failed: {potential_domain}")

        # Step 2: Try corrected spellings for common typos
        corrected_name = self._fix_common_typos(normalized_name)
        if corrected_name != normalized_name:
            print(f"    Domain Discovery: Trying typo-corrected name '{corrected_name}' ...")
            domain, verified, search_notes = self.search_google_for_domain(corrected_name)
            if domain and verified:
                notes.append(f"Found via typo correction: {search_notes}")
                return domain, True, "; ".join(notes)

        # Step 3: Attempt to infer and verify a domain based on common patterns
        print(f"    Domain Discovery: Attempting to verify potential domains for '{company}' ...")
        domain, verified, search_notes = self.search_google_for_domain(normalized_name)
        notes.append(search_notes)
        if domain and verified:
            return domain, True, "; ".join(notes)

        # Step 4: Fallback to pattern inference (unverified guess)
        print(f"    Domain Discovery: Falling back to pattern inference for '{company}' ...")
        inferred_domain, inferred_verified, pattern_notes = self.infer_domain_pattern(normalized_name)
        notes.append(pattern_notes)

        if inferred_domain:
            return inferred_domain, inferred_verified, "; ".join(notes)

        return None, False, "; ".join(notes)

    def _fix_common_typos(self, company: str) -> str:
        ...
        for typo, correction in typo_map.items():
            if typo in lower_company:
                return company.lower().replace(typo, correction).title()

        return company

    def _generate_domain_variations(self, company_clean: str) -> List[str]:
        """Generate multiple domain variations including shorter forms"""
        variations = []

        # Full cleaned name
        variations.append(company_clean)

        # Remove common business words to get shorter domains
        words = company_clean.split()
        if len(words) > 1:
            # Try removing filler words
            filler_words = {'capital', 'partners', 'group', 'holdings', 'investments',
                          'management', 'companies', 'corporation', 'infrastructure'}
            filtered_words = [w for w in words if w.lower() not in filler_words]

            if filtered_words and len(filtered_words) != len(words):
                # Create shortened version without filler words
                variations.append(''.join(filtered_words))

            # Try first word only if it's substantial (4+ chars)
            if len(words[0]) >= 4:
                variations.append(words[0])

        return variations

    def search_google_for_domain(self, company: str) -> Tuple[Optional[str], bool, str]:
        """
        Attempt to infer and verify a domain using pattern matching with variations.
        Returns: (domain or None, verified_bool, notes)
        """
        try:
            clean_company = company.strip().lower()
            if not clean_company:
                return None, False, "Empty company name after trimming."

            # Remove non-alphanumeric characters except spaces/hyphens
            company_clean = re.sub(r'[^\w\s-]', '', clean_company)
            ...
            if not company_clean:
                return None, False, f"No usable tokens found in '{company}'."

            # Generate multiple variations (full name, shorter versions)
            variations = self._generate_domain_variations(company_clean)

            tried_domains = []
            # Try multiple TLDs for each variation
            for variation in variations:
                for tld in ['.com', '.net', '.org', '.io', '.dk', '.global']:
                    potential_domain = f"{variation}{tld}"
                    tried_domains.append(potential_domain)
                    verified, cleaned = self._verify_domain(potential_domain)
                    if verified and cleaned:
                        return cleaned, True, f"Verified domain via pattern attempt: {cleaned}"

            return None, False, f"No verified domain found. Tried: {', '.join(tried_domains[:10])}{'...' if len(tried_domains) > 10 else ''}"

        except Exception as e:
            return None, False, f"Domain inference error: {str(e)}"

    def infer_domain_pattern(self, company: str) -> Tuple[Optional[str], bool, str]:
        """
        Try a simple pattern-based domain inference and optionally verify it.
        Returns: (domain or None, verified_bool, notes)
        """
        try:
            ...
            if not clean:
                return None, False, "Pattern inference failed (empty cleaned company name)."
            ...
            if not clean:
                return None, False, "Pattern inference failed (only business suffixes present)."

            domain = f"{clean}.com"
            verified, cleaned = self._verify_domain(domain)
            if verified and cleaned:
                return cleaned, True, f"Pattern inference verified domain: {cleaned}"

            return domain, False, f"Pattern inference (unverified guess): {domain}"

        except Exception as e:
            return None, False, f"Pattern inference error: {str(e)}"

    def _verify_domain(self, domain: str) -> Tuple[bool, Optional[str]]:
        """
        Attempt to verify whether a domain resolves and is not a parking page.
        Returns: (verified_bool, cleaned_domain or None)
        """
        if not domain:
            return False, None
        ...
        if normalized in self._domain_cache:
            verified, cleaned = self._domain_cache[normalized]
        ...
        parking_domains = ['domainmarket', 'sedo', 'godaddy', 'afternic', 'hugedomains', 'dan.com']

        schemes = ['https://', 'http://']
        for scheme in schemes:
            url = f"{scheme}{normalized}"
            try:
                # Use GET instead of HEAD for better parking page detection
                response = requests.get(url, allow_redirects=True, timeout=5, headers={
                ...
                if response.status_code < 400:
                    ...
                    if any(parking in host for parking in parking_domains):
                        self._domain_cache[normalized] = (False, None)
                        return False, None

                    # Check response content for parking indicators (first 2000 chars)
                    ...
                    # Looks legitimate
                    if host:
                        self._domain_cache[normalized] = (True, host)
                        return True, host
...
class SalesforceClassifier:
    ...
    def search_by_domain(self, domain: str) -> Optional[Dict]:
        """Search Salesforce Accounts by domain with flexible matching"""
        if not self.sf or not domain:
            return None

        try:
            # Extract the base domain name (without .com, .net, etc.)
            domain_base = domain.split('.')[0] if '.' in domain else domain

            # Try multiple search variations
            search_variations = [
                domain,  # Exact: aligned.com
                domain_base,  # Base: aligned (will match aligneddc.com, aligned.com, etc.)
            ]

            for search_term in search_variations:
                soql_query = f"SELECT Id, Name, Website, Customer_Designation__c, Owner.Name, LastActivityDate, SystemModstamp FROM Account WHERE Website LIKE '%{search_term}%'"

                print(f"    Salesforce: Searching for domain '{search_term}' in Account websites...")
                result = self.sf.query(soql_query)

                if result.get('records'):
                    # Return first account match
                    record = result['records'][0]
                    print(f"    Salesforce: Found domain match '{record.get('Name')}' with website containing '{search_term}'")
                    return {
                        'type': 'Account',
                        'id': record['Id'],
...
class CompanyProcessor:
    ...
    def process_file(self, csv_file_path: str):
        ...
        # Initialize progress (reset counters for each run)
        progress = {
        ...
        for i, company_data in enumerate(companies):
            ...
            if domain and domain_verified:
                ...
                print(f"    ⚠️ Domain inference (unverified): {domain}")
                progress['domain_stats']['unverified'] += 1
            ...
            classification, details = self.sf_classifier.classify_company(company_name, domain if domain_verified else None)
            ...
            company_with_details['domain_notes'] = domain_notes
            company_with_details['domain_verified'] = domain_verified
            if not domain_verified and 'domain_guess' in company_data:
                company_with_details['domain_guess'] = company_data['domain_guess']
        ...
        print(
            "🌐 Domain Discovery: "
            f"{progress['domain_stats']['found']} verified, "
            f"{progress['domain_stats']['unverified']} unverified guesses, "
            f"{progress['domain_stats']['not_found']} not found"
        )
