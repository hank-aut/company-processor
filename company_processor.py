...
class CompanyProcessor:
    ...
    def process_file(self, csv_file_path: str):
        ...
        # Phase 3: Batch AI Relevancy Scoring (for No SF Match and Qualified Prospects)
        companies_to_score = results['no_salesforce_match'] + results['salesforce_qualified']

        if companies_to_score:
            print(f"\nPhase 3: AI Relevancy Scoring ({len(companies_to_score)} companies)...")
            ...
            try:
                ...
                # Batch score all companies at once (10 per API call)
                ...
                for classification in ['no_salesforce_match', 'salesforce_qualified']:
                    for company in results[classification]:
                        ...
                        if company_name in scoring_results:
                            ...
                            print(f"  ✅ {company_name}: {ai_result['score']}/5 - {ai_result['category']}")

            except Exception as e:
                print(f"  ⚠️  AI Scoring failed: {str(e)}")
                # Mark all as N/A if batch scoring fails
                for classification in ['no_salesforce_match', 'salesforce_qualified']:
                    for company in results[classification]:
                        company['ai_score'] = 'N/A'
                        company['ai_category'] = 'N/A'
                        company['ai_use_case'] = 'N/A'
                        company['ai_reasoning'] = 'Scoring unavailable'
